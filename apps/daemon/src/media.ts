...
import { downstreamMediaRenderers } from './downstream/index.js';
import {
  ensureProject,
  kindFor,
  mimeFor,
...
export async function generateMedia(args: {
  ...
  try {
    if (
      ...
      providerNote = result.providerNote;
      suggestedExt = result.suggestedExt;
    } else if (downstreamMediaRenderers[`${def.provider}:${surface}`]) {
      const result = await downstreamMediaRenderers[`${def.provider}:${surface}`]!(ctx, credentials, args.onProgress);
      bytes = result.bytes;
      providerNote = result.providerNote;
      suggestedExt = result.suggestedExt;
    } else {

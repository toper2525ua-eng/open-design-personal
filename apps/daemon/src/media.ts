...
// ---------------------------------------------------------------------------
// Provider: OpenRouter — OpenAI-compatible gateway to many upstreams.
//
// Two transport shapes are used here:
//   * Image  → POST /chat/completions with body
//              { model, messages:[{role:'user',content:prompt}], modalities:['image','text'] }
//              The response carries images at choices[0].message.images[].image_url.url
//              as a `data:image/...;base64,…` URL we decode to bytes.
//   * Video  → POST /videos with body { model, prompt, aspect_ratio, duration?, resolution?, generate_audio? }.
//              Returns { id, polling_url, status }. We poll the polling_url until status='completed',
//              then download the first entry of `unsigned_urls[]`.
//
// Default base URL is https://openrouter.ai/api/v1 (overridable). Auth is
// `Authorization: Bearer <OPENROUTER_API_KEY>`. We also send the optional
...
async function renderOpenRouterVideo(ctx: MediaContext, credentials: ProviderConfig, onProgress?: ProgressFn): Promise<RenderResult> {
  if (!credentials.apiKey) {
    throw new Error(
      'no OpenRouter API key — configure it in Settings or set OD_OPENROUTER_API_KEY',
    );
